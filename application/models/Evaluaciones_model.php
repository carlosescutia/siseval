...
class Evaluaciones_model extends CI_Model {
    ...
    public function get_evaluacion($id_evaluacion, $cve_dependencia, $cve_rol) {
        ...
        $sql = 'select e.* from evaluaciones e left join proyectos py on e.cve_proyecto = py.cve_anterior_proyecto left join programas pg on py.cve_programa = pg.cve_programa where e.id_evaluacion = ? and py.cve_dependencia::text LIKE ?';
